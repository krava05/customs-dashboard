...
if not check_password():
    st.stop() # Если пароль неверный, останавливаем выполнение

# Если пароль верный, выполняем основной код
...
# --- АУТЕНТИФИКАЦИЯ ---
try:
    # В Cloud Run аутентификация происходит автоматически через сервисный аккаунт
    if os.environ.get('K_SERVICE'):
        client = bigquery.Client(project=PROJECT_ID)
    # При локальном запуске используется JSON-ключ
    else:
